  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-3">
        {courses.map((c) => (
          <div key={c.id} className="bg-surface border border-line rounded-xl p-5 transition-colors duration-300">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-muted text-xs mt-2 font-mono">
                  {c.students?.length || 0} student(s) enrolled
                </p>
              </div>
              <button
            ...
            {enrollingId === c.id && (
              <form onSubmit={(e) => handleEnroll(e, c.id)} className="flex gap-2 mt-4 pt-4 border-t border-line">
                <input
                  type="email"
                  value={enrollEmail}
